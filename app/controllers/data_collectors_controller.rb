class DataCollectorsController < ApplicationController
  ...
  def index
    @data_collectors = DataCollector.all
  ...
  def create
    ...
    respond_to do |format|
      if @data_collector.save
        format.html { redirect_to(@data_collector, :notice => 'Data collector was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @data_collector.update_attributes(params[:data_collector])
        format.html { redirect_to(@data_collector, :notice => 'Data collector was successfully updated.') }
        format.xml  { head :ok }
      else
